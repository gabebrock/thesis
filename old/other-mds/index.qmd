...
required_packages <- c("tidyverse", "readxl", "mapview", "sf", "tinytext", "textdata", "kableExtra", "rJava", "xlsx")
...
# count total number of 2024 stops for calculations
total_obs <- nrow(sqf2024)

# race-blind treat all hispanic stops
sqf2024 <- sqf2024 %>%
  mutate(SUSPECT_RACE_DESCRIPTION = case_when(SUSPECT_RACE_DESCRIPTION %in% c("BLACK HISPANIC", "WHITE HISPANIC") ~ "HISPANIC",
...
# load yearly aggregates for stop outcomes (2002-2024)
stopsOverTime <- read_csv("r-data/nypd-stop/stops_2002_2024.csv")

# time series for all stop outcomes
stopsTrend <- stopsOverTime %>%
  pivot_longer(cols = c(totalStops, noOutcome, arrestSummons),
  ...
  labs(
    title = "Stop-and-Frisk Outcomes Over Time",
    x = "Year",
    y = "Number of Stops",
    fill = "Outcome"
  ) +
...
# save time series to image file
ggsave(filename = "images/stopsTrend.svg", plot = stopsTrend, width = 6.5, height = 4, units = "in")
...
sqf2024summary <- sqf2024 %>%
  summarize(
    ...
    count_hispanic = sum(SUSPECT_RACE_DESCRIPTION == "HISPANIC", na.rm = TRUE),
...
blackHeatMap <- ggplot(nyc_census2020) +
  geom_sf(aes(fill = black_prop), color = "white", linewidth = 0.3) +
...
# load counts for misdemeanor offenses, by precinct
...
plot_grid(blackHeatMap, blackStopHeatMap)
...
top10disparatePrecincts <- nypd_sf %>%
  arrange(desc(stopDisparity)) %>%
  slice_head(n = 10) %>%
  select(!c('Shape__Area', 'Shape__Length', 'geometry'))

# slice 10 least stop disparate precincts
bottom10disparatePrecincts <- nypd_sf %>%
  arrange(stopDisparity) %>%
  slice_head(n = 10) %>%
  select(!c('Shape__Area', 'Shape__Length', 'geometry'))

# find 10 most normal precincts by stop disparity
median10disparatePrecincts <- nypd_sf %>%
  arrange(abs(stopDisparity - median(stopDisparity, na.rm = TRUE))) %>%
  slice_head(n = 10) %>%
  select(!c('Shape__Area', 'Shape__Length', 'geometry'))
...
frisksByRace2024 <- sqf2024 %>%
  ...
  drop_na(SUSPECT_SEX) %>%
  filter(SUSPECT_RACE_DESCRIPTION %in% c("BLACK", "HISPANIC", "WHITE")) %>%
  count() %>%
  ungroup() %>%
  mutate(prop = round(n / sum(n), 2))
...
# need to figure out a way to make sure the token that's selected isn't erroneous/unrelated to demeanor (i.e. "officer", "and", "to", etc.)
...
```{r age-height-weight of suspects}
...
cameraIntersectionAggregate <- read_csv("amnesty/amnesty-data/counts_per_intersections.csv", 
                              col_types = cols(Lat = col_double(), 
                                               Long = col_double(), 
                                               ImageDate = col_date(format = "%Y-%m"))) %>%
  mutate(
    Lat = format(Lat, scientific = FALSE, digits = 10),
    Long = format(Long, scientific = FALSE, digits = 10)
  )
...
sqf2024 %>%
  filter(STOP_LOCATION_PRECINCT %in% top10disparatePrecincts$Precinct) %>%
  group_by(SUSPECT_RACE_DESCRIPTION, SUSPECT_ARRESTED_FLAG) %>%
  count()


%>%
  group_by(STOP_WAS_INITIATED, SUSPECT_RACE_DESCRIPTION, SUSPECT_ARRESTED_FLAG) %>%
  count()
...
sqf2024 %>%
  filter(STOP_LOCATION_PRECINCT %in% top10disparatePrecincts$Precinct) %>%
  group_by(SUSPECT_RACE_DESCRIPTION) %>%
  select(STOP_LOCATION_PRECINCT, STOP_WAS_INITIATED, SUSPECT_RACE_DESCRIPTION, FRISKED_FLAG, SEARCHED_FLAG, SUSPECT_ARRESTED_FLAG, SUMMONS_ISSUED_FLAG, DEMEANOR_OF_PERSON_STOPPED)

```

```{r}

# days with the most activity, by stops per race
# Testing out the theory that CompStat has an impact on officer behavior. 
# The concentration patterns are not identical between ethnicities. Black people are stopped the most on Tuesdays, the day before CompStat meetings, followed by Th, Fr, and Saturday.

sqf2024 %>% # all stops in 2024
  group_by(SUSPECT_RACE_DESCRIPTION, DAY2, MONTH2) %>% 
  count() %>% # count number of Black stops, for each weekday, for each month
  ungroup(SUSPECT_RACE_DESCRIPTION) %>%
  mutate(prop = n / sum(n)) %>% # calculate the Black proportion of all of the stops for that day
  filter(SUSPECT_RACE_DESCRIPTION == "BLACK") %>% # just show black stops
  arrange(desc(prop)) %>% # arrange days based on highest proportion of all stops
  head(n = 25) %>% # chop top 25 days out the year
  ungroup(MONTH2) %>% 
  count(DAY2) %>% # count the number of days in the top 25
...
sqf2024 %>%
  ...
  ungroup(SUSPECT_RACE_DESCRIPTION) %>%
  mutate(prop = n / sum(n)) %>%
  filter(SUSPECT_RACE_DESCRIPTION == "HISPANIC") %>%
  arrange(desc(prop)) %>%
  head(n = 25) %>%
  ungroup(MONTH2) %>%
  count(DAY2) %>%
...
sqf2024 %>%
  ggplot(aes(x = STOP_FRISK_DATE)) +
  geom_line(stat = "count") + 
  geom_vline(
    xintercept = sqf2024$STOP_FRISK_DATE[sqf2024$DAY2 == "Wednesday"],
    linetype = "dotted", # Optional: use a dashed line for clarity
    color = "blue",      # Optional: set a custom color
    alpha = 0.5          # Optional: make the lines semi-transparent
  ) +
  facet_wrap(~ SUSPECT_RACE_DESCRIPTION, nrow = 3)
...
# ----------------------------------------------------
# 1. Create crime-type classification
# ----------------------------------------------------

violent_crimes <- c(
  "ASSAULT", "ROBBERY", "MENACING", "MURDER",
  "FORCIBLE TOUCHING", "RAPE", "RECKLESS ENDANGERMENT",
  "TERRORISM"
)
...
property_crimes <- c(
  "GRAND LARCENY", "PETIT LARCENY", "GRAND LARCENY AUTO",
  "BURGLARY", "CPSP", "CRIMINAL TRESPASS",
  "CRIMINAL MISCHIEF", "UNAUTHORIZED USE OF A VEHICLE",
  "AUTO STRIPPING", "AUTO STRIPPIG",
  "CRIMINAL POSSESSION OF FORGED INSTRUMENT",
  "THEFT OF SERVICES", "MAKING GRAFFITI"
)
...
sqf_clean <- sqf_hist %>%
  filter(YEAR2 == 2024) %>%
  mutate(
    month = month(STOP_FRISK_DATE),

    race = case_when(
      str_detect(SUSPECT_RACE_DESCRIPTION, "BLACK") ~ "Black",
      str_detect(SUSPECT_RACE_DESCRIPTION, "HISPANIC") ~ "Hispanic",
      str_detect(SUSPECT_RACE_DESCRIPTION, "WHITE") ~ "White",
      TRUE ~ NA_character_
    ),

    crime_type = case_when(
      SUSPECTED_CRIME_DESCRIPTION %in% violent_crimes  ~ "Violent crimes",
      SUSPECTED_CRIME_DESCRIPTION %in% weapons_crimes  ~ "Weapons crimes",
      SUSPECTED_CRIME_DESCRIPTION %in% property_crimes ~ "Property crimes",
      SUSPECTED_CRIME_DESCRIPTION %in% drug_crimes     ~ "Drug crimes",
      TRUE ~ NA_character_
    )
  ) %>%
  filter(!is.na(race), !is.na(crime_type))

# ----------------------------------------------------
# 3. Aggregate: count by crime x month x race
# ----------------------------------------------------

monthly_counts <- sqf_clean %>%
  mutate(
    arrest = if_else(SUSPECT_ARRESTED_FLAG == "Y", "Arrest", "No arrest")
  ) %>%
  group_by(crime_type, month, race, arrest) %>%
  summarise(n_stops = n(), .groups = "drop")

# ----------------------------------------------------
# 4. Plot (matching the original layout)
# ----------------------------------------------------

ggplot(monthly_counts, aes(x = month, y = n_stops, linetype = race)) +
  geom_line(size = 0.8) +
  scale_y_log10(breaks = c(200, 300, 1000, 3000)) +
  scale_x_continuous(breaks = 1:12, limits = c(1, 12)) +
  facet_grid(crime_type ~ arrest) +
  coord_fixed(ratio = 11/6) +
  labs(x = "month", y = "# stops") +
