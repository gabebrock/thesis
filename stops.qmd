...
sqf2024 <- sqf2024 %>%
  ...
  filter(SUSPECT_RACE_DESCRIPTION != "(null)", !is.na(SUSPECT_RACE_DESCRIPTION))
...
frisksByRace2024 <- sqf2024 %>%
  ...
  filter(SUSPECT_SEX != "(null)", !is.na(SUSPECT_SEX)) %>%
...
# create subtotals by race
subtotals <- searchesByRace2024 %>%
  group_by(SUSPECT_SEX, SUSPECT_RACE_DESCRIPTION) %>%
  summarise(n = sum(n), .groups = "drop") %>%
  mutate(SEARCHED_FLAG = "Subtotal")

# combine racial subtotals with original frisks table 
searchesByRace2024 <- bind_rows(searchesByRace2024, subtotals) %>%
  arrange(match(SEARCHED_FLAG, c("Y", "N", "Subtotal")),
          match(SUSPECT_SEX, c("MALE", "FEMALE")),
          SUSPECT_RACE_DESCRIPTION)
...
afinnSentiment <- get_sentiments("afinn")


stopDemeanorSentiment <- stopDemeanorDesc2024 %>%
  full_join(get_sentiments("afinn"), join_by(DEMEANOR == word)) %>%
  count(value) 
